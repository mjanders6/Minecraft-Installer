...
class MC_Installer:
    ...
    def mc_install(username, jar_file, password):
        first_commands = [
            ...
            # subprocess.Popen(f'sed -i `s/eula=.*/eula=true/` /opt/{username}/server/eula.txt', shell=True, stdin=None),
            subprocess.run(['sed', '-i', 's/eula=.*/eula=true/', f'/opt/{username}/server/eula.txt'], check=True),
            # print('Changing the rcon port. \n'),
            # subprocess.Popen(f'sed -i `s/rcon.port=.*/rcon.port=25575/` /opt/{username}/server/server.properties', shell=True, stdin=None),
            # print('Enable rcon. \n'),
            # subprocess.Popen(f'sed -i `s/enable-rcon=.*/enable-rcon=true/` /opt/{username}/server/server.properties', shell=True, stdin=None),
            # print('Adding the password to rcon. \n'),
            # subprocess.Popen(f'sed -i `s/rcon.password=.*/rcon.password={password}/` /opt/{username}/server/server.properties', shell=True, stdin=None),
